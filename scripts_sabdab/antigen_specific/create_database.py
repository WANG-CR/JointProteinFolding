    ...
    with open(file_name, "r") as f:
        ...
        for line in f.readlines():
            ...
            if td.get("antigen_type", None) in possible_antigen_type:
                ...
                sabdab_dict[split_line[0]].append(td)
...
def truncate_antibody_pdb(
    ...
    trunc_pdb_path_prefix = os.path.join(output_dir, pdb_id)
    cnt = 0
    pdb_text = ''
    try:
    ...
    if sabdab_summary_file is not None:
        ...
        lchains = []
        antigen_chains = []
        for tmp_dict in sabdab_summary_file[pdb_id]:
            hchains.append(tmp_dict["Hchain"])
            lchains.append(tmp_dict["Lchain"])
            antigen_chains.append(tmp_dict.get("antigen_chain", 'NA'))
    ...
    for (hchain, lchain, antigen_chain) in zip(hchains, lchains, antigen_chains):
        ...
        if len(antigen_text) == 0:
            ...
            continue
        cnt += 1
        trunc_pdb_path = trunc_pdb_path_prefix + f"_{cnt}.pdb"
...
def main(args):
    ...
    # is_protein = summary_df['antigen_type'].isin(['protein', 'protein | protein'])

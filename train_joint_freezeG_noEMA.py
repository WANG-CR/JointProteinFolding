...
from openfold.model.esm.misc import extract_bb
import debugger
...
class OpenFoldWrapper(pl.LightningModule):
    def __init__(self, config):
        ...
        for key, params in self.f_model.named_parameters():
            if params.requires_grad:
                logging.info(f"key that requires gradient: {key}")
    ...
    def forward_joint(self, batch):
        ...
        final_atom_positions = extract_bb(f_outputs)
        ...
        h_outputs = self.g_model.forward_h(batch, final_atom_positions)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ## log training loss
        loss_breakdown = {}
        self._log(loss_breakdown, batch, f_outputs)


        # calculate reconstruction loss
        # only loss for training
    ...
    def validation_step(self, batch, batch_idx):
        ...
        f_breakdown = {}
        self._log(f_breakdown, batch, f_outputs, train=False)

        # compute g loss
        logits = h_outputs["sm"]["seqs_logits"][-1]
        aatype = batch["aatype"]
        ...
        # Log it
        self.log('val/reconstruction_PPL', ppl, on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)    # show
        self.log('val/reconstruction_AAR', aars, on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)  
        self.log(
                f"val/loss", 
                ce, 
    ...
    def configure_optimizers(self) -> torch.optim.Adam:
        ...
        optimizer = torch.optim.Adam(
            [{"params":filter(lambda p: p.requires_grad, self.f_model.parameters())}],
            lr=optim_config.lr,

...
def gather_jobs(fasta_path, mode=0):
    ...
    if mode == 0:
        ...
        for i, (seq, desc) in enumerate(zip(input_seqs, input_descs)):
            tag = desc[:4].lower()
            chain = desc[-1].upper()
            assert chain in ['H', 'L'], f'only accept H chain and L chain, found {chain}'
        ...
        for f in os.listdir(fasta_path):
            ...
            if (ext == ".fasta"):
                ...
                jobs.append([tag, len(input_seqs[0]), len(input_seqs[1]), input_seqs[0] + input_seqs[1]])
            else:
                continue
...
def main(args):
    ...
    for job in jobs:
        ...
        if args.residue_embedding_dir is not None:
            local_residue_embedding_dir = os.path.join(args.residue_embedding_dir, tag)
        else:

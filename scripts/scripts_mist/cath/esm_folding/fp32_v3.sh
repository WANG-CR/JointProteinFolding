...
#SBATCH --output=/scratch/b/bengioy/chuanrui/backup/alphafold/output/slurm_log/corrige_fp32_v3.out
#SBATCH --error=/scratch/b/bengioy/chuanrui/backup/alphafold/output/slurm_log/corrige_fp32_v3.err
...
TORCH_DISTRIBUTED_DEBUG=INFO srun python train_protein.py $TRAIN_DIR $OUTPUT_DIR \
    ...
    --wandb_version corrige_fp32_v3 \
    --wandb_project corrige_pf \
    --train_epoch_len 1000 \
    --resume_from_ckpt $OUTPUT_DIR/corrige_pf/replace_esmFold_lowLR-corrige_fp32_v2/checkpoints/epoch41-step10499-val_loss=2.700.ckpt \
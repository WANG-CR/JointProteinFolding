...
#SBATCH --output=/scratch/b/bengioy/chuanrui/backup/alphafold/output_esm/slurm_log/esmfold_lr1e-3.out
#SBATCH --error=/scratch/b/bengioy/chuanrui/backup/alphafold/output_esm/slurm_log/esmfold_lr1e-3.err
...
TORCH_DISTRIBUTED_DEBUG=INFO srun python train_esmfold.py $TRAIN_DIR $OUTPUT_DIR \
    ...
    --seed 2024 \
    --yaml_config_preset yaml_config/esmFold_8+8_lr1e-3.yml \
    --precision 32 --gpus 4 --log_every_n_steps 50 \
    --wandb true \
    --wandb_entity chuanrui \
    --wandb_version esmfold_lr1e-3 \

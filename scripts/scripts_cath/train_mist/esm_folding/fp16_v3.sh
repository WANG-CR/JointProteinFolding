...
#SBATCH --output=/scratch/b/bengioy/chuanrui/backup/alphafold/output/slurm_log/corrige_fp16_v3.out
#SBATCH --error=/scratch/b/bengioy/chuanrui/backup/alphafold/output/slurm_log/corrige_fp16_v3.err
...
# continuously training after corrige_fp16_v2
...
TORCH_DISTRIBUTED_DEBUG=INFO srun python train_protein.py $TRAIN_DIR $OUTPUT_DIR \
    ...
    --wandb_version corrige_fp16_v3 \
    --wandb_project corrige_pf \
    --train_epoch_len 1000 \
    --resume_from_ckpt $OUTPUT_DIR/corrige_pf/replace_esmFold_lowLR-corrige_fp16_v2/checkpoints/epoch47-step11999-val_loss=2.595.ckpt
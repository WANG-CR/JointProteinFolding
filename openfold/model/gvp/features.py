...
from openfold.model.gvp.gvp_utils import flatten_graph
from openfold.model.gvp.util import normalize, norm, nan_to_num, rbf
from openfold.model.gvp.gvp_modules import GVP, LayerNorm
    ...
    def get_node_features(coords, coord_mask, with_coord_mask=True):
        ...
        if with_coord_mask:
            node_scalar_features = torch.cat([
                ...
                coord_mask.to(dtype=node_scalar_features.dtype).unsqueeze(-1)
            ], dim=-1)
        # vector features
        X_ca = coords[:, :, 1]
...
class GVPGraphEmbedding(GVPInputFeaturizer):
    ...
    def forward(self, coords, coord_mask, padding_mask):
        with torch.no_grad():
            ...
            #print(f"graph embed coords type: {coords_dtype}!!")

            if coords_dtype != torch.float32:
                #print(f"explitly convert node feat")
                coords = coords.to(dtype=torch.float32)

            node_features = self.get_node_features(coords, coord_mask)
            check_inf_nan(node_features)
            edge_features, edge_index = self.get_edge_features(
                coords, coord_mask, padding_mask)
            check_inf_nan(edge_features)
            check_inf_nan(edge_index)
            #print(f"edge_index type: {edge_index.dtype}")
            if coords_dtype != torch.float32:
                #print(f"explitly convert gvp out")
                node_features = (
                    node_features[0].to(dtype=coords_dtype),
                    node_features[1].to(dtype=coords_dtype),
                )
                edge_features = (
                    edge_features[0].to(dtype=coords_dtype),
                    edge_features[1].to(dtype=coords_dtype),
                )
        #print(f"node feature scalar type: {node_features[0].dtype}")
        #print(f"node feature vector dtype: {node_features[1].dtype}")
        node_embeddings_scalar, node_embeddings_vector = self.embed_node(node_features)
        #print(f"node_embeddings_scalar type: {node_embeddings_scalar.dtype}")
        #print(f"node_embeddings_vector dtype: {node_embeddings_vector.dtype}")
        edge_embeddings = self.embed_edge(edge_features)

        node_embeddings = (
            node_embeddings_scalar,

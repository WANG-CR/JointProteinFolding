...
def make_seq_mask(protein, loop_type=None):
    ...
    if loop_type == "CDR":
        ...
        protein["loop_mask"] = (protein["loop_index"] == int(loop_type)).to(torch.float32)
    ...
    # print(f'loop mask is {protein["loop_mask"]}')
    return protein
...
def add_constant_field(protein, key, value):
    protein[key] = torch.tensor(value, device=protein["aatype"].device)
    return protein
